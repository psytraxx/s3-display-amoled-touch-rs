    ...
    DELAY: DelayNs,
{
    ...
    /// to `f`. The slice references `self.buf` directly — no heap allocation.
    /// Returns `Some(f(data))` on success, `None` if the command produced no ACK.
    async fn config_command<F, T>(&mut self, cmd: Command, f: F) -> Result<Option<T>, LD2410Error>
    where
        F: FnOnce(&[u8]) -> T,
    {
